                    ...
                    <div class="carousel-inner">
                        <c:forEach items="${favList}" var="fav" varStatus="status">
                            <c:if test="${status.count % 3 == 1}">
                                <div class="carousel-item <c:if test="${status.count == 1}"> active </c:if>" style="width: 93%;">
                                <div style="display: inline-flex; width: 100%;">
                            </c:if>
                            <div class="brand-card"
                                 style="float: left; margin-left: auto; margin-right: auto; width: 245px; border: 0;">
                                <div class="brand-card-header" style="width: 400px; height:250px;position: relative; background-size: 100%; background-image: url('${fav.imgUrl}')">
                                    <a class="xB" href="/favorite/delete/${fav.favoriteId}"
                                       style="margin-top: -200px; margin-right: -340px; z-index: 99999">
                                        <img src="/img/x-btn.png" style="width: 20px; height: 20px;">
                                    </a>
                                    <div class="brand-card-body">
                                        <a href="/restaurant/${fav.restaurantId}"
                                           style="margin-left: auto; margin-right: auto;">
                                            <div class="chart-wrapper" style="text-align: center;">
                                                <div style="margin-top: 10%; padding: 5%; background-color: rgba( 255, 255, 255, 0.6);">
                                                    <span style="color: #000000; font-size: 25px; font-weight: 600;">${fav.title}</span>
                                                    <div style="color: #000000; font-size: 15px;text-overflow:ellipsis; white-space:nowrap; word-wrap:normal;overflow:hidden;margin-top: 10px">
                                                            ${fav.address}
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <c:if test="${status.count % 3 == 0 or status.count == favList.size()}">
                                </div>
                                </div>
                            </c:if>
                        </c:forEach>
